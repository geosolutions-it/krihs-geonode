...
RUN mkdir -p /usr/src/krihs_geonode
...
COPY . /usr/src/krihs_geonode/
WORKDIR /usr/src/krihs_geonode
...
RUN chmod +x /usr/src/krihs_geonode/tasks.py \
    && chmod +x /usr/src/krihs_geonode/entrypoint.sh
...
ENTRYPOINT service cron restart && service memcached restart && /usr/src/krihs_geonode/entrypoint.sh
